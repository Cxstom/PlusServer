...
namespace Plus.Communication.Packets.Incoming.Groups.Forums
{
    class PostMessageEvent : IPacketEvent
    {
        public void Parse(GameClient session, ClientPacket packet)
        {
            ...
            {
                session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.post.message.cooldown"));
                return;
            }
            ...
            {
                session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.post.content_empty"));
                return;
            }

            if (message.Length < 10)
            {
                session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.post.message.too_short"));
                return;
            }
            ...
            {
                session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.post.message.content.contains_flagged_words"));
                return;
            }
            ...
            {
                session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.post.content.length.too_long"));
                return;
            }
            ...
            if (threadId == 0)
            {
                if (!session.GetHabbo().GetPermissions().HasRight("group_forum_membership_override"))
                {
                    ...
                    {
                        session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.permissions.create_thread.member_admin_creator_only"));
                        return;
                    }
                    else if (group.GetForum().ThreadCreationSetting == 2 && !group.IsAdmin(session.GetHabbo().Id) && group.CreatorId != session.GetHabbo().Id)
                    {
                        session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.permissions.create_thread.admin_creator_only"));
                        return;
                    }
                    else if (group.GetForum().ThreadCreationSetting == 3 && group.CreatorId != session.GetHabbo().Id)
                    {
                        session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.permissions.create_thread.creator_only"));
                        return;
                    }
                }
                ...
                if (group.GetForum().TryAddThread(newThreadId, newThread))
                {
                    ...
                    {
                        group.GetForum().AddScore(Convert.ToInt32(PlusEnvironment.GetSettingsManager().TryGetValue("forum.post_thread.score")));
                        session.GetHabbo().GetStats().ForumPosts += 1;
                        session.GetHabbo().LastForumMessageUpdateTime = DateTime.Now;
                        session.SendPacket(new ThreadCreatedComposer(group, newThread));
                    }
                }
                else
                {
            ...
            else
            {
                ...
                if (!session.GetHabbo().GetPermissions().HasRight("group_forum_membership_override"))
                {
                    ...
                    {
                        session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.permissions.create_post.admin_creator_only"));
                        return;
                    }
                    else if (group.GetForum().PostCreationSetting == 3 && group.CreatorId != session.GetHabbo().Id)
                    {
                        session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.permissions.create_post.creator_only"));
                        return;
                    }
                }
                ...
                if (message.Length < 5)
                {
                    session.SendNotification(PlusEnvironment.GetLanguageManager().TryGetValue("forum.post.reply.message.too_short"));
                    return;
                }
                ...
                group.GetForum().AddScore(Convert.ToInt32(PlusEnvironment.GetSettingsManager().TryGetValue("forum.post_message.score")));
                session.GetHabbo().GetStats().ForumPosts += 1;
                session.GetHabbo().LastForumMessageUpdateTime = DateTime.Now;
                session.SendPacket(new ThreadReplyComposer(group, thread, post));
            }
        }
    }
}