    ...
    class GetThreadDataEvent : IPacketEvent
    {
        public void Parse(GameClient session, ClientPacket packet)
        {
            ...
            if (thread.Deleted)
            {
                ...
                {
                    session.SendPacket(new RoomNotificationComposer("forums.error.access_denied"));
                    return;
                }
            ...
            session.SendPacket(new ThreadDataComposer(group, thread, thread.GetPosts().Skip(startIndex).Take(thread.GetPosts().Count - startIndex).Take(20).ToList(), startIndex));
        }
    }
}
