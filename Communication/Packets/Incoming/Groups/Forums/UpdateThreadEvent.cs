    ...
    class UpdateThreadEvent : IPacketEvent
    {
        public void Parse(GameClient session, ClientPacket packet)
        {
            ...
            if (Thread.Locked != locked)
            {
                ...
                session.SendPacket(new RoomNotificationComposer(Thread.Locked ? "forums.thread.locked" : "forums.thread.unlocked"));
            }
            else if (Thread.Pinned != pin)
            {
                Thread.Pinned = !Thread.Pinned;
                session.SendPacket(new RoomNotificationComposer(Thread.Pinned ? "forums.thread.pinned" : "forums.thread.unpinned"));
            }
